...
type
  TMain = class(TForm)
    ...
    procedure BtnCrashLogClick(Sender: TObject);
  private
...
{ ---- Crash log example ---------------------------------------------------------------------------
  ...
  1. DebugEngine.HookException (in the uses clause) makes every exception carry the addresses of the
     call stack captured at raise. Nothing else is needed for E.StackTrace.
  2. DebugEngine.CrashLog turns an exception into a madExcept-like text report: header (machine,
     OS, memory, process, executable, callstack crc, exception) + optional sections.
  3. Symbols come from the detailed .map next to the executable (Linking > Map file = Detailed),
     converted in memory, or from a SMAP embedded with "Insert debug info" / the DD tool. }
...
procedure TMain.BtnCrashLogClick(Sender: TObject);
var
  LogFile: string;
begin
  LogMem.Clear;
  ...
  try
    ProcessOrder(0); // raises a few calls deep
  except
    on E: Exception do
      { WriteCrashLog appends the report to LogFile and returns it.
      ...
      LogMem.Text := WriteCrashLog(E, LogFile, [crsHeader, crsException, crsThreads]);
  end;
  LogMem.Lines.Add('Report appended to: ' + LogFile);
end;
